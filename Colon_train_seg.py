...
from src.colon_loader import get_dataloader_colon as get_dataloader
...
def train_one_epoch(
    ...
    for metric_name in metrics:
        ...
        for i in range(len(config.colon_loader.checkModels)):
            metric_dice[f"Train/ {config.colon_loader.checkModels[i]}"] = float(
                batch_acc[i]
            )
...
def val_one_epoch(
    ...
    for metric_name in metrics:
        ...
        for i in range(len(config.colon_loader.checkModels)):
            metric_dice[f"{flag}/ {config.colon_loader.checkModels[i]}"] = float(
                batch_acc[i]
            )
        metric.update(metric_dice)
...
if __name__ == "__main__":
    ...
    if config.finetune.Colon.checkpoint != "None":
        checkpoint_name = config.finetune.Colon.checkpoint
    else:
        checkpoint_name = (
    ...
    inference = monai.inferers.SlidingWindowInferer(
        roi_size=config.colon_loader.target_size,
        overlap=0.5,
